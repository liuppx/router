...
11. `identity.trust_dir`：Node 信任包目录，默认 `/data/node`；插件钱包身份登录用它本地校验钱包身份 presentation 和 JWT-VC 凭证，不实时调用 Node。
12. `identity.node_url`、`identity.app_id`：仅用于无钱包插件时的钱包身份授权码登录。`identity.callback_url` 可选，用于 Router 对外回调地址与 `server.public_url` 不同时。留空时使用 `/api/v1/public/oauth/identity/callback`。该回调地址必须与 Node 应用的 `redirectUris` 精确一致。契约不使用 Passport assertion 或 `subjectId`。
