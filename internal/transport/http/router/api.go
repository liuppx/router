...
func SetApiRouter(engine *gin.Engine) {
	...
	adminRouter.Use(middleware.GlobalAPIRateLimit())
	{
		...
		adminBillingRoute.Use(middleware.AdminAuth())
		{
			...
			adminBillingRoute.GET("/finance/consistency/issues", adminbilling.GetFinanceConsistencyIssues)
		...
		adminChannelRoute.Use(middleware.AdminAuth())
		{
			...
			adminChannelRoute.POST("/:id/billing/usage/sync", channel.SyncChannelProviderUsage)
